...
export default function BotsTradingPage() {
  const { bots, getBot, userSubscriptions, getUserSubscriptions, buySubscription, isBuyingSubscription } =
  ...
  }, [getBot, getUserSubscriptions]);

  const subscribedBotIds = useMemo(() => {
    return new Set((userSubscriptions?.bots || []).map((b) => b._id));
  }, [userSubscriptions]);
  ...
  const traders = useMemo(() => {
    ...
    return bots.map((bot) => ({
      id: bot._id,
      name: bot.name,
      handle: `@${bot.name.replace(/\s+/g, "").toLowerCase()}`,
      price: bot.price, // FIXED price
      followers: bot.stats?.followers ?? 0,
      roi: bot.stats?.roi ?? 0,
      winRate: bot.stats?.winRate ?? 0,
      equity: bot.stats?.equity ?? 0,
      meta: bot.meta,
      description: bot.description,
      isSubscribed: subscribedBotIds.has(bot._id),
    }));
  }, [bots, subscribedBotIds]);
            ...
            >
              ...
              <div className="mt-4 flex items-center gap-2">
                <button
                  ...
                  className={`flex-none px-3 py-2 rounded-lg bg-linear-to-tr from-secondary ${t.isSubscribed ? "opacity-50" : "opacity-100"} text-white text-sm flex items-center gap-2`}
                >
